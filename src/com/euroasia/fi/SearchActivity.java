...
public class SearchActivity extends ListActivity {
	...
	private class RetrieveSearch extends AsyncTask<Void, Void, Void>{
		...
		protected void onCancelled() {		
			...
			TextView failresults = (TextView)findViewById(R.id.failresults);
			failresults.setText("Haku on peruutettu");
		...
		protected void onPreExecute() {
			...
			cancelDialog.setMessage("Hakee kappaleita...");
		...
		protected void onPostExecute(Void result) {
			if(ia == 0 && fail == 0){
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					failresults.setText("Sinulla ei näytä olevan toimivaa Internet-yhteyttä tai signaali on heikko. Ole hyvä ja käynnistä wifi tai tiedot ja yritä uudelleen.");
		        }else{
					failresults.setText("Tuloksia ei löytynyt");
		        }
			}else if(ia == 0 && fail > 0){
				fail = 0;
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					...
					failresults.setText("Haussa näyttää tapahtuneen virhe. Ole hyvä ja yritä uudelleen.");
		        }
			}else{
				setListAdapter(adapter);
			}
		...
		public void getDilandau(String search) throws IOException, MalformedURLException, NullPointerException{
			...
			if(!sb.toString().contains("Kappaleita ei löytynyt")){
