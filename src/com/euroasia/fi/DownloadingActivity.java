...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("Ladattuja Yhteensä: "+totaldownloading);
        }else{
        	text.setText("Ladattuja Yhteensä: 0");
        }
        ...
        this.clearall.setOnClickListener(new OnClickListener() {
        	public void onClick(View v) {
        		...
        		toastmake("Nollaa valmiit & epäonnistuneet lataukset");
        	}
        });
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("Latausta odotellaan");
		    		}else if(a == 0){
		    			progress.setText("Lataus on valmis");
		    		}else if(a == -1){
		        		progress.setText("Lataus epäonnistui");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				builder.setMessage("Latausvaihtoehto")
				       .setCancelable(false)
				       .setNegativeButton("Poista", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				                mDbHelper.deleteDownload(ida);
				                refreshlist();
				                toastmake("MP3-lataus poistettu");
				           }
				       })
				       .setNeutralButton("Uudelleenyritys", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   mDbHelper.updateDownloaded(ida, "1"); 
				        	   refreshlist();
				        	   toastmake("Aseta uudelleenyritykseen");
				           }
				       })
				...
				final CharSequence[] items = {"Avaa MP3", "Aseta soittoääneksi", "Lataa uudelleen", "Poista", getString(R.string.btn_cancel_name)};
	
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("Valitse vaihtoehto");
				builder.setItems(items, new DialogInterface.OnClickListener() {
				    public void onClick(DialogInterface dialog, int item) {
				    	if(items[item] == "Avaa MP3"){
				    	    try {
				    	        //launch intent
				    	    	...
				    	    	toastmake("Virhe MP3-latauksessa");
				    	        e.printStackTrace();
				    	    }
				    	}else if(items[item] == "Aseta soittoääneksi"){
							...
							builder.setMessage("Haluatko varmasti poistaa: " + title + " ?")
							       ...
							       .setPositiveButton("Kyllä", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	    ...
							        	    mDbHelper.deleteDownload(ida);
							        	    refreshlist();
							                toastmake("MP3 poistettu");
							           }
							       })
							       .setNegativeButton("Ei", new DialogInterface.OnClickListener() {
							           public void onClick(DialogInterface dialog, int id) {
							        	   toastmake("MP3:a ei poistettu");
							               dialog.cancel();
							           }
							       });
							AlertDialog alert = builder.create();
							alert.show();
				    	}else if(items[item] == "Lataa uudelleen"){
				    		mDbHelper.updateDownloaded(ida, "1"); 
				    		refreshlist();
				    		toastmake("Aseta uudelleenyritykseen");
				    	}
				...
				builder.setMessage("Latausvaihtoehto")
				       .setCancelable(false)
				       .setNegativeButton("Pysäytä lataus", new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
				        	   if(sid == 1){
				                ...
				                toastmake("MP3-lataus pysäytetty");
				           }
				       })
				       .setPositiveButton(getString(R.string.btn_cancel_name), new DialogInterface.OnClickListener() {
				           public void onClick(DialogInterface dialog, int id) {
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "Lataa parhaillaan", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "Latausta odotellaan", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "Valmiit lataukset", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "Epäonnistunut lataus", 100));
